...
async def build_statistics(db: Session, user_id: int, tmdb: TMDBClient) -> dict[str, Any]:
    ...
    for entry in diary_entries:
        if entry.movie_id in movie_cache:
            continue
        try:
            movie_cache[entry.movie_id] = await tmdb.movie_details(entry.movie_id)
    ...
    return {
        "total_movies_watched": len(diary_entries),
...
async def _recent_activity(db: Session, user_id: int, tmdb: TMDBClient) -> list[dict[str, Any]]:
    ...
    for item in activity:
        try:
            movie = await tmdb.movie_summary(item["movie_id"])
            item["movie"] = {"id": movie.get("id"), "title": movie.get("title"), "poster_path": movie.get("poster_path")}
        except HTTPException as exc:
            if exc.status_code == 503:
                break
            raise
