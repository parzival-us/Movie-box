    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 border-b border-white/[0.08] bg-black/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="flex max-w-full gap-1 overflow-x-auto rounded-lg border border-white/[0.08] bg-white/[0.03] p-1" aria-label="Main navigation">
            {links.map((link) => {
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    [
                      ...
                      isActive ? "bg-white text-black" : "text-white/50 hover:bg-white/[0.08] hover:text-white",
                    ].join(" ")
                  }
                  title={link.label}
